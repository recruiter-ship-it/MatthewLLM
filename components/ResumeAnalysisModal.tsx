...
const ResumeAnalysisModal: React.FC<{ resume: CandidateResume; onClose: () => void }> = ({ resume, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-md flex items-center justify-center z-50 p-4" onClick={onClose}>
            <div
                className="relative bg-white/80 backdrop-blur-2xl border border-white/30 w-full max-w-3xl rounded-3xl shadow-2xl p-6 sm:p-8 animate-zoom-in text-slate-800 max-h-[90vh] overflow-y-auto"
            ...
            >
                ...
                <h2 className="text-2xl font-bold text-slate-900 mb-2">Анализ резюме</h2>
                <p className="text-slate-600 mb-6 break-words font-medium">{resume.fileName}</p>
